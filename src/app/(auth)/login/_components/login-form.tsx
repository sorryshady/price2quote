...
const LoginForm = () => {
  ...
  const resendVerificationEmail = async () => {
    if (!userInfo) return;
    const token = await generateToken(userInfo.email, userInfo.id, 'email-verification');
    ...
    setIsResending(true);
    try {
      const response = await fetch(`${env.NEXT_PUBLIC_API_URL}/api/send-email`, {
        method: 'POST',
        body: JSON.stringify({
          html,
        ...
        <AlertDialog open={showNotVerified} onOpenChange={setShowNotVerified}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Your email has not yet been verified. Please check your email for a verification
                link. The link expires in 15 minutes. You may resend the verification email by
                clicking the button below.
              </AlertDialogDescription>
              <AlertDialogFooter className="mt-4">
                <AlertDialogCancel className="cursor-pointer">Cancel</AlertDialogCancel>
                <Button variant="default" onClick={resendVerificationEmail} disabled={isResending}>
